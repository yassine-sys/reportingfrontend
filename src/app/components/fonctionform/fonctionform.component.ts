...
export class FonctionformComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.submoduleService.getAllSubModules().subscribe(
      (data: (number | SubModule)[]) => {
        // Specify the type of 'data' as an array of 'number' or 'SubModule'
  ...
  private _filterSubmodules(value: string): any[] {
    ...
    return this.submodules.filter((submodule) =>
      submodule.subModuleName.toLowerCase().includes(filterValue)
    );
  }
