...
export class FunctionChartsComponent implements OnInit, OnDestroy {
  ...
  onDrop(event: CdkDragDrop<any[]>) {
    ...
    this.functionService
      .updateReportOrderForFunction(this.funcId, repId, newOrder)
      .subscribe(
        () => {
  ...
  updateChartData(data: any[]): void {
    data.forEach(
      (filteredChartData: { title: any; list_de_donnees: any[][] }) => {
          ...
          if (chartData.operator === true) {
            chartData.list_de_donnees.forEach((row: any[]) => {
              ...
              const operator = operatorList.find(
                (operatorObj: any) => operatorObj.id === operatorId
              );
              if (operator) {
  ...
  refreshAPICall() {
    ...
    this.route.params.subscribe((params) => {
      ...
      this.filtredSubscription = this.filterService
        ...
        .subscribe((filtred) => {
          ...
          if (
            ...
            this.chartService.getFunctionChartFiltred(filtred).subscribe(
              (data: any) => {
                if (data) {
      ...
      this.d = this.chartService
        .getFunctionChart(this.funcId)
        .subscribe((data: any) => {
          if (data) {
            this.chartData = data;
